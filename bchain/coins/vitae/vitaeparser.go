...
func init() {
	// Vitae mainnet Address encoding magics
	MainNetParams = chaincfg.MainNetParams
	MainNetParams.Net = MainnetMagic
	MainNetParams.PubKeyHashAddrID = []byte{71} // starting with 'v'
	MainNetParams.ScriptHashAddrID = []byte{13}
	MainNetParams.PrivateKeyID = []byte{212}

	// Vitae testnet Address encoding magics
...
// VitaeParser handle
type VitaeParser struct {
	*btc.BitcoinParser
	baseparser                         *bchain.BaseParser
	BitcoinOutputScriptToAddressesFunc btc.OutputScriptToAddressesFunc
}

// NewVitaeParser returns new VitaeParser instance
func NewVitaeParser(params *chaincfg.Params, c *btc.Configuration) *VitaeParser {
	p := &VitaeParser{
...
// GetChainParams contains network parameters for the main Vitae network
...
func (p *VitaeParser) ParseBlock(b []byte) (*bchain.Block, error) {
	r := bytes.NewReader(b)
	w := wire.MsgBlock{}
	h := wire.BlockHeader{}
	err := h.Deserialize(r)
...
func (p *VitaeParser) PackTx(tx *bchain.Tx, height uint32, blockTime int64) ([]byte, error) {
	return p.baseparser.PackTx(tx, height, blockTime)
}

// UnpackTx unpacks transaction from protobuf byte array
func (p *VitaeParser) UnpackTx(buf []byte) (*bchain.Tx, uint32, error) {
	return p.baseparser.UnpackTx(buf)
}

// ParseTx parses byte array containing transaction and returns Tx struct
func (p *VitaeParser) ParseTx(b []byte) (*bchain.Tx, error) {
	t := wire.MsgTx{}
	r := bytes.NewReader(b)
...
func (p *VitaeParser) TxFromMsgTx(t *wire.MsgTx, parseAddresses bool) bchain.Tx {
...
func (p *VitaeParser) ParseTxFromJson(msg json.RawMessage) (*bchain.Tx, error) {
	var tx bchain.Tx
...
func (p *VitaeParser) outputScriptToAddresses(script []byte) ([]string, bool, error) {
...
func (p *VitaeParser) IsAddrDescIndexable(addrDesc bchain.AddressDescriptor) bool {
...
func (p *VitaeParser) GetAddrDescForUnknownInput(tx *bchain.Tx, input int) bchain.AddressDescriptor {
	if len(tx.Vin) > input {
...
func (p *VitaeParser) GetValueSatForUnknownInput(tx *bchain.Tx, input int) *big.Int {
	if len(tx.Vin) > input {
		scriptHex := tx.Vin[input].ScriptSig.Hex
		if scriptHex != "" {
			script, _ := hex.DecodeString(scriptHex)
...
func (p *VitaeParser) GetValueSatFromZerocoinSpend(signatureScript []byte) (*big.Int, error) {
...
func (p *VitaeParser) P2CSScriptToAddress(script []byte) ([]string, bool, error) {
	if len(script) < 50 {
